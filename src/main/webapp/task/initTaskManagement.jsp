...
<%
	...
	if(operation.equals("create")){
		...
		for(Node logNode : logNodeList){
			...
			for(Node baseValueNode : baseValueNodeList){
				...
				if(baseNode.getTextContent().equals("title")){
					...
					if (!previousNode.getTextContent().equals("")) {
						UUID previousStatId = DataAccessFactory
								.getInstance().createUUID(
										previousNode.getTextContent());
						Stat previousStat = flow
								.getStat(previousStatId);
						if (previousStat != null) {
							previousNode.setTextContent(previousStat
									.getName());
						}
					}

					if (!currentNode.getTextContent().equals("")) {
						UUID currentStatId = DataAccessFactory
								.getInstance().createUUID(
										currentNode.getTextContent());
						Stat currentStat = flow.getStat(currentStatId);
						if (currentStat != null) {
							currentNode.setTextContent(currentStat
									.getName());
						}
					}
				}
			...
			for (Node extValueNode : extValueNodeList) {
				...
				if (previousNode.getTextContent().length() > 0) {
					...
					if (field.getType().equals(Type.t_selection)) {
						String[] optionIdStrArray = previousNode
								.getTextContent().split("\\,");
						for (String optionIdStr : optionIdStrArray) {
							UUID optionId = DataAccessFactory
									.getInstance().createUUID(
											optionIdStr);
						...
						String[] referIdStrArray = previousNode
								.getTextContent().split("\\,");

						String[] referTitleArray = new DataAccessSessionMySQL()
								.queryFieldByIds(referIdStrArray,
										"title", null);
						...
						for (String referTitleStr : referTitleArray) {
							...
							.equals(Type.t_attachment)) {
						String[] attachIdStrArray = previousNode
								.getTextContent().split("\\,");
						Set<UUID> allAttachSet = new HashSet<UUID>();

						for (int i = 0; i < attachIdStrArray.length; i++) {
							UUID uuid = DataAccessFactory.getInstance()
									.createUUID(attachIdStrArray[i]);
							if (uuid != null)
								allAttachSet.add(uuid);
						}

						Map<String, String> attachIdNameMap = new AttachmentAccessSessionMySQL()
								.queryAttachmentIdNames(allAttachSet
										.toArray(new UUID[0]));
				...
				if (currentNode.getTextContent().length() > 0) {
					...
					if (field.getType().equals(Type.t_selection)) {
						String[] optionIdStrArray = currentNode
								.getTextContent().split("\\,");
						for (String optionIdStr : optionIdStrArray) {
							UUID optionId = DataAccessFactory
									.getInstance().createUUID(
						...
						String[] referIdStrArray = currentNode
								.getTextContent().split("\\,");
						...
						String[] attachIdStrArray = currentNode
								.getTextContent().split("\\,");
						Set<UUID> allAttachSet = new HashSet<UUID>();
		...
		List<Node> taskFieldNodeList = XMLUtil.getNodes(taskFieldsNode,
				"field");
		for (Node taskFieldNode : taskFieldNodeList) {
			String fieldIdStr = XMLUtil.getSingleNodeTextContent(
					taskFieldNode, "id");
			fieldDataMap.put(fieldIdStr, new LinkedHashSet<String>());

			List<Node> fieldDataNodeList = XMLUtil.getNodes(
					taskFieldNode, "data");
			for (Node fieldDataNode : fieldDataNodeList) {
				fieldDataMap.get(fieldIdStr).add(
						fieldDataNode.getTextContent());
			}
		}
		...
		xml += templateXml.substring(templateXml.indexOf("<layout>"),
				templateXml.indexOf("</layout>"));
		xml += "</layout></root>";
		...
		List<Node> fieldRowNodeList = XMLUtil.getNodes(doc,
				"root/layout/rows/row");
		for (Node fieldRowNode : fieldRowNodeList) {
			List<Node> fieldColumnNodeList = XMLUtil.getNodes(
					fieldRowNode, "column");
			for (Node fieldColumnNode : fieldColumnNodeList) {
				List<Node> fieldNodeList = XMLUtil.getNodes(
						fieldColumnNode, "field");

				for (Node fieldNode : fieldNodeList) {
					Node typeNode = XMLUtil.getSingleNode(fieldNode,
							"type");
					typeNode.setTextContent(typeNode.getTextContent()
							.split("\\_")[1]);

					Node dataTypeNode = XMLUtil.getSingleNode(
							fieldNode, "dataType");
					if (dataTypeNode.getTextContent().length() > 0)
						dataTypeNode.setTextContent(dataTypeNode
								.getTextContent().split("\\_")[1]);

					List<Node> optionNodeList = XMLUtil.getNodes(
							fieldNode, "options/option");
					Node fieldIdNode = XMLUtil.getSingleNode(fieldNode,
							"id");
					String fieldId = fieldIdNode.getTextContent();

					for (Node optionNode : optionNodeList) {
						Node forbiddenNode = XMLUtil.getSingleNode(
								optionNode, "forbidden");
						forbiddenNode.setTextContent(forbiddenNode
								.getTextContent().split("\\_")[1]);
					}

					///////////////////////////////////设置对应项目////////////////////////////////////////////////////////////////////
					if (template.isProTemplate()
							&& XMLUtil.getSingleNodeTextContent(
									fieldNode, "name").equals("对应项目")) {
						XMLUtil.removeAll(XMLUtil.getSingleNode(
								fieldNode, "options"));
						Field productField = template.getField("对应产品");
						if (productField != null) {
							UUID optionId = task
									.getSingleSelection(productField
											.getId());
							if (optionId != null) {
								Map<String, String> allProjects = ProjectInvolveManager
										.getInstance().getProjectMap(
												key.getUsername(),
												optionId.getValue());

								Node optionsNode = XMLUtil
										.getSingleNode(fieldNode,
												"options");
								int i = 0;
								for (String productId : allProjects
										.keySet()) {
									Node optionNode = doc
											.createElement("option");
									Node optionIdNode = doc
											.createElement("id");
									optionIdNode
											.setTextContent(productId);
									optionNode
											.appendChild(optionIdNode);

									Node optionNameNode = doc
											.createElement("name");
									optionNameNode
											.setTextContent(allProjects
													.get(productId));
									optionNode
											.appendChild(optionNameNode);

									Node controlOptionIdNode = doc
											.createElement("controlOptionId");
									optionNode
											.appendChild(controlOptionIdNode);

									Node forbiddenNode = doc
											.createElement("forbidden");
									forbiddenNode
											.setTextContent("permit");
									optionNode
											.appendChild(forbiddenNode);

									Node indexOrderNode = doc
											.createElement("indexOrder");
									indexOrderNode
											.setTextContent(Integer
													.toString(++i));
									optionNode
											.appendChild(indexOrderNode);
					...
					Node controlHiddenFieldsIdsNode = XMLUtil
							.getSingleNode(fieldNode,
									"controlHiddenFields");
					List<Node> controlHiddenFieldsIdList = XMLUtil
							.getNodes(controlHiddenFieldsIdsNode,
									"controlHiddenField");
					for (Node controlHiddenField : controlHiddenFieldsIdList) {
						...
						controlHiddenFields.append(controlHiddenField
								.getTextContent());
					}
					XMLUtil.removeAll(controlHiddenFieldsIdsNode);
					controlHiddenFieldsIdsNode
							.setTextContent(controlHiddenFields
									.toString());

					StringBuffer controlHiddenStates = new StringBuffer();
					Node controlHiddenStatesIdsNode = XMLUtil
							.getSingleNode(fieldNode,
									"controlHiddenStates");
					List<Node> controlHiddenStatesIdList = XMLUtil
							.getNodes(controlHiddenStatesIdsNode,
									"controlHiddenState");
					for (Node controlHiddenState : controlHiddenStatesIdList) {
						...
						controlHiddenStates.append(controlHiddenState
								.getTextContent());
					}
					XMLUtil.removeAll(controlHiddenStatesIdsNode);
					controlHiddenStatesIdsNode
							.setTextContent(controlHiddenStates
									.toString());
					///////////////////////////////////////////////////////////////////////////////////////////////////////////////////

					//controlActionIds
					StringBuffer controlActionIds = new StringBuffer();
					Node controlActionIdsNode = XMLUtil.getSingleNode(
							fieldNode, "controlActionIds");
					List<Node> controlActionIdNodeList = XMLUtil
							.getNodes(controlActionIdsNode,
									"controlActionId");
					for (Node controlActionIdNode : controlActionIdNodeList) {
						...
						controlActionIds.append(controlActionIdNode
								.getTextContent());
					}

					XMLUtil.removeAll(controlActionIdsNode);
					controlActionIdsNode
							.setTextContent(controlActionIds.toString());

					//controlRoleIds
					StringBuffer controlRoleIds = new StringBuffer();
					Node controlRoleIdsNode = XMLUtil.getSingleNode(
							fieldNode, "controlRoleIds");
					List<Node> controlRoleIdNodeList = XMLUtil
							.getNodes(controlRoleIdsNode,
									"controlRoleId");
					for (Node controlRoleIdNode : controlRoleIdNodeList) {
						...
						controlRoleIds.append(controlRoleIdNode
								.getTextContent());
					}

					XMLUtil.removeAll(controlRoleIdsNode);
					controlRoleIdsNode.setTextContent(controlRoleIds
							.toString());

					Node datasNode = doc.createElement("datas");
					fieldNode.appendChild(datasNode);

					String fieldIdStr = XMLUtil
							.getSingleNodeTextContent(fieldNode, "id");
					if (fieldDataMap.containsKey(fieldIdStr)) {
						...
						for (String data : fieldDataMap.get(fieldIdStr)) {
							if (typeNode.getTextContent().equals(
									"reference")) {
								UUID referenceId = DataAccessFactory
										.getInstance().createUUID(data);
								Data reference = das
										.queryData(referenceId);
								if (reference == null)
									continue;

								if (templateMap.get(reference
										.getTemplateId()) == null) {
									Template temp = das
											.queryTemplate(reference
													.getTemplateId());
									if (temp != null)
										templateMap.put(temp.getId(),
												temp);
									Flow flow1 = das.queryFlow(temp
											.getFlowId());
									if (flow1 != null)
										flowMap.put(flow1.getId(),
												flow1);
								}

								if (referTemplateDataMap.get(reference
										.getTemplateId()) == null)
									referTemplateDataMap.put(
											reference.getTemplateId(),
											new HashSet<String>());

								referTemplateDataMap.get(
										reference.getTemplateId()).add(
										data);
							} else if (typeNode.getTextContent()
									.equals("attachment")) {
						...
						if (referTemplateDataMap.keySet().size() > 0) {
							...
							for (UUID templateIdTmp : referTemplateDataMap
									.keySet()) {
								StringBuffer referSql = new StringBuffer();
								referSql.append("select id,templateId, statusId,title ");
								Field piorityField = templateMap.get(
										templateIdTmp)
										.getField("修改优先级");
								if (piorityField != null) {
									templatePiorityField
											.put(templateIdTmp,
													piorityField);
									piorityFieldColName = FieldNameCache
											.getInstance()
											.getFieldName(
													piorityField
															.getId(),
									...
									if (piorityFieldColName != null)
										referSql.append(",").append(
												piorityFieldColName);
								}
								referSql.append(" from ").append(
										TableRuleManager.getInstance()
												.getDataTableName(
														templateIdTmp));

								referSql.append(" where id in ( ");
								for (String referDataId : referTemplateDataMap
										.get(templateIdTmp)) {
									referSql.append(referDataId)
											.append(",");
								}
								...
								allReferDataList
										.addAll(DbPoolConnection
												.getInstance()
												.getResultSetListBySql(
														referSql.toString()));
							}
							...
							for (Map<String, String> dataMap : allReferDataList) {
								...
								datasNode.appendChild(dataNode);
								String statusName = "";
								statusName = statusMap.get(dataMap
										.get("statusid"));
								UUID templateUUID = DataAccessFactory
										.getInstance()
										.createUUID(
												dataMap.get("templateid"));
								Flow tmpFlow = flowMap.get(templateMap
										.get(templateUUID).getFlowId());
								if (statusName == null) {
									Stat status = tmpFlow
											.getStat(DataAccessFactory
													.getInstance()
													.createUUID(
															dataMap.get("statusid")));
									if (status != null) {
								...
								String priorityName = "";
								if (piorityFieldColName != "") {
									priorityName = priorityMap
											.get(dataMap
													.get(piorityFieldColName));
									if (priorityName == null) {
										Option priority = templatePiorityField
												.get(templateUUID)
												.getOption(
														DataAccessFactory
																.getInstance()
																.createUUID(
																		dataMap.get(piorityFieldColName)));
										if (priority != null) {
								...
								statusName = statusName == null ? ""
										: statusName;
								dataNode.setTextContent(dataMap
										.get("id")
										+ "&|;"
										+ dataMap.get("title")
										+ "&|;"
										+ (statusName.equals("") ? null
												: statusName)
										+ "&|;"
										+ (priorityName.equals("") ? null
												: priorityName));
							}
						...
						Map<String, String> attachIdNameMap = new AttachmentAccessSessionMySQL()
								.getAttachIdAndNameByIds(attachIdSet);

						for (String attachId : attachIdNameMap.keySet()) {
							Node dataNode = doc.createElement("data");
							datasNode.appendChild(dataNode);
							dataNode.setTextContent(attachId + "&|;"
									+ attachIdNameMap.get(attachId));
						}
					}
				}
				try {
