...
public class BackRightAccessSessionMySQL {
	...
	public List<UserInfo> getBackRightUsers() {
		...
		try
		{
			...
			String sql = "SELECT A.user_name, B.id,B.nick_name from event_user  A JOIN user_info  B on A.user_name = B.user_name order by A.user_name asc";
	...
	public List<UserInfo> getTemplateRightUser(String templateId) {
		...
		{
			conn = DbPoolConnection.getInstance().getReadConnection();
			String sql = "SELECT A.admin_user, B.id,B.nick_name from template_admin_user  A left JOIN user_info  B on A.admin_user = B.user_name where A.template_id = ? order by A.admin_user asc";
	...
	public boolean addUserTemplateRight(String[] templateIds, String userMail) {
		...
		try
		{
			...
			String sql = "insert into template_admin_user(template_id,admin_user) values(?,?)";
			pstmt = conn.prepareStatement(sql);
	...
	public Map<String, String> queryUserTemplateRights(String userMail) {
		...
		{
			conn = DbPoolConnection.getInstance().getReadConnection();
			String sql = "SELECT A.admin_user, B.id,B.name from template_admin_user A left JOIN template B on A.template_id = B.id where A.admin_user = ?";
