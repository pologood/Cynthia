...
import com.sogou.qadev.service.cynthia.util.ByteToInputStream;
...
public class AttachmentAccessSessionMySQL
{
	...
	public Attachment createAttachment(String name, String username, byte[] data)
	{
		...
		try{
			...
			pstm = conn.prepareStatement("insert into attachment(id,name,create_user,create_time,\"SIZE\",file_id,\"DATA\") values(?,?,?,?,?,?,?)");//empty_blob()
			pstm.setString(1, id.getValue());
			pstm.setString(2, name);
			pstm.setString(3, username);
			pstm.setTimestamp(4, createTime);
			pstm.setLong(5, dataLength);
			pstm.setString(6, fileId);
			pstm.setObject(7, data);
	...
	public Attachment[] queryAttachments(UUID[] ids, boolean needData)
	{
		...
		try{
			...
			for(int i = 0; i < ids.length; i++){
				if (ids[i] == null)
						continue;
				statement.setString(i + 1,ids[i].getValue());
			}
			...
			while (rs.next()){
				...
				if (needData){
					...
					if(data == null){
						...
						data = ByteToInputStream.input2byte(ins);
					}
	...
	public boolean updateAttachment(Attachment attachment) {
		...
		try{
			...
			pstm.setObject(6, attachment.getData());
	...
	public boolean deleteAttachment(String ids){
		if(ids == null || "".equals(ids)) return false;
		String[] idArray = ids.split(",");
		StringBuffer sqlBuffer = new StringBuffer();
		sqlBuffer.append("delete from attachment where id in (");
		for (String attachId : idArray) {
			sqlBuffer.append("'").append(attachId).append("'").append(",");
		}
		sqlBuffer.deleteCharAt(sqlBuffer.length() -1 );
		sqlBuffer.append(")");
		...
		try
		{
			conn = DbPoolConnection.getInstance().getConnection();
			stat = conn.createStatement();
			return stat.executeUpdate(sqlBuffer.toString()) > 0 ? true : false;
			
		}catch(Exception e){
			e.printStackTrace();
		}finally
		{
			DbPoolConnection.getInstance().closeAll( stat, conn);
		}
