...
public class TimerActionAccessSessionMySQL
{
	...
	public ErrorCode addTimerAction(TimerAction timerAction)
	{
		...
		try
		{
			...
			pstm = conn.prepareStatement("INSERT INTO timer_action(id,name,class_name,method,create_user,param) values(?,?,?,?,?,?)");

			pstm.setString(1, timerAction.getId().getValue());
	...
	public TimerAction[] queryStatisticByUser(String userName) {
		...
		{
			conn = DbPoolConnection.getInstance().getReadConnection();
			pstm = conn.prepareStatement("SELECT * FROM timer_action where is_public = 1 or create_user = ? order by is_public desc");
			...
			while(rst.next())
			{
				...
				UUID id = DataAccessFactory.getInstance().createUUID(uuid);
				TimerAction timerAction = new TimerActionImpl(id);
