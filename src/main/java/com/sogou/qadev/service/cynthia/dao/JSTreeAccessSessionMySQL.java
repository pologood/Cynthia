...
public class JSTreeAccessSessionMySQL {
	...
	public int addNode(int parentId,int position,String title,String userName)
	{
		...
		try{
			...
			String uuid = new UUIDAccessSessionMySQL().getUUID();
			
			String sql = "insert into tree (parent_id,position,title,user_name,id) values (?,?,?,?,?)";
			pstm1 = conn.prepareStatement(sql);
			pstm1.setInt(1, parentId);
			pstm1.setInt(2, position);
			pstm1.setString(3, title);
			pstm1.setString(4, userName);
			pstm1.setString(5, uuid);
			pstm1.execute();
			result = Integer.valueOf(uuid);
			return result;
		}catch(Exception e)
		{
			e.printStackTrace();
			...
			DbPoolConnection.getInstance().closeAll(rs, pstm, conn);
			DbPoolConnection.getInstance().closeAll(rs, pstm1, conn);
		}
	}
	...
	public String[] getFavorateFilters(String userName)
	{
		...
		try{
			...
			{
				String createSql = "insert into favorite_filters (user_name,filters,id) values (?,?,SEQ_CYNTHIA.nextval)";
