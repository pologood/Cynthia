...
private static Logger logger = Logger.getLogger(TemplateLogAccessSessionMySQL.class.getName());
	...
	public boolean addTemplateAccessLog(TemplateOperateLog templateOperateLog)
	{
		...
		try
		{
			...
			String sql = "insert into template_operate_log (template_id,field_id,field_name,operate_type,create_time,create_user,before_xml,after_xml,id) values (?,?,?,?,?,?,?,?,SEQ_CYNTHIA.nextval)";
