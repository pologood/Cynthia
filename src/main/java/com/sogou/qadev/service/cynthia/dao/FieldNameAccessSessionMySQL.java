...
public class FieldNameAccessSessionMySQL {
	...
	public Map<String, String> queryTemplateFieldMap(String templateId){
		...
		try
		{
			...
			while(rs.next())
			{
				allFieldColNames.put(rs.getString("fieldColName").toLowerCase(), rs.getString("fieldId"));  //通过fieldId 可找到fieldColName
			}
		}catch(Exception e)
		{
			e.printStackTrace();
	...
	public boolean addFieldColName(String templateId,String fieldColName,String fieldId,String fieldType)
	{
		...
		try
		{
			...
			sql = "insert into field_name_map(templateId,fieldColName,fieldId,fieldType,id) values(?,?,?,?,SEQ_CYNTHIA.nextval)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, templateId);
	...
	public boolean updateFieldColName(String fieldId,String fieldColName)
	{
		...
		{
			conn = DbPoolConnection.getInstance().getConnection();
			String sql = "update field_name_map set fieldColName  = ? where fieldId = ?";
			pstmt = conn.prepareStatement(sql);
