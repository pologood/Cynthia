...
public class TagAccessSessionMySQL {
	...
	public boolean addTagData(String tagId , String[] dataIds)
	{
		...
		try{
			...
			pstmt = conn.prepareStatement("insert into tag_data (tag_id,tag_data) values (?,?)");
			for (String dataId : dataIds) {
				if(!checkTagData(tagId,dataId)){
	...
	public boolean checkTagData(String tagId,String dataId){
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try{
			conn = DbPoolConnection.getInstance().getConnection();
			pstmt = conn.prepareStatement("select tag_id from tag_data where tag_id=? and tag_data=?");
			pstmt.setString(1, tagId);
			pstmt.setString(2, dataId);
	...
	public int addTag(String userName , String tagName , String tagColor)
	{
		...
		try
		{
			conn = DbPoolConnection.getInstance().getConnection();
			String uuid = "";
			String seq_sql = "select SEQ_CYNTHIA.nextval from dual";
			seq_ptmt =  conn.prepareStatement(seq_sql);
			seq_rs = seq_ptmt.executeQuery();
			if(seq_rs.next()){
				uuid = String.valueOf(seq_rs.getInt(1));
			}
			String sql = "insert into tag (name,user_name,color,id) values (?,?,?,?)";
			pstm = conn.prepareStatement(sql);
			pstm.setString(1, tagName);
			pstm.setString(2, userName);
			pstm.setString(3, tagColor);
			pstm.setString(4,uuid);
			pstm.executeUpdate();
			result = Integer.parseInt(uuid);
		}catch(Exception e)
		{
			e.printStackTrace();
		}finally
	...
	public Map<String, String> getUserTagDataMap(String userName) {
		...
		{
			conn = DbPoolConnection.getInstance().getReadConnection();
			String sql = "SELECT A.id , A.name,A.user_name ,B.tag_data,B.filter_id from tag A LEFT JOIN " +
					"tag_data B on A.id = B.tag_id where A.user_name=? order by B.tag_data";
			pstm = conn.prepareStatement(sql);
			pstm.setString(1, userName);
			rs = pstm.executeQuery();
	...
	public List<TagBean> getDataTags(String userName, String dataId) {
		...
		{
			conn = DbPoolConnection.getInstance().getConnection();
			String sql = "SELECT A.id , A.name,A.user_name ,A.color,B.tag_data,B.filter_id from tag  A LEFT JOIN " +
					"tag_data  B on A.id = B.tag_id where A.user_name=? and B.tag_data=? order by B.tag_data";
			pstm = conn.prepareStatement(sql);
			pstm.setString(1, userName);
